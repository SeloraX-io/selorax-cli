...
module.exports = async function appDeploy(opts) {
  ...
  // Warn about sandbox extensions that have neither entry nor sandbox_url
  const incompleteSandboxExts = config.extensions.filter(
    (e) => e.mode === 'sandbox' && !e.entry && !e.sandbox_url
  );
  if (incompleteSandboxExts.length > 0) {
    console.log(chalk.yellow('\n  Warning: sandbox extension(s) with no entry and no sandbox_url:'));
    for (const ext of incompleteSandboxExts) {
      console.log(chalk.yellow(`    • ${ext.extension_id || ext.title}: provide "entry" (for build) or "sandbox_url" (pre-hosted)`));
    }
    console.log();
  }
