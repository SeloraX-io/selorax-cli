...
module.exports = async function authLogin(opts) {
  ...
  // Block private/internal IP ranges (allow localhost/127.0.0.1 for local dev)
  {
    const { hostname } = new URL(apiUrl);
    if (hostname !== 'localhost' && hostname !== '127.0.0.1') {
      const privatePatterns = [/^10\./, /^192\.168\./, /^172\.(1[6-9]|2\d|3[01])\./, /^0\.0\./, /^169\.254\./, /^100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\./];
      if (privatePatterns.some((r) => r.test(hostname))) {
        console.error(chalk.red('  Private/internal API URLs are not allowed.'));
        process.exit(1);
      }
    }
  }
